...
\usepackage{pgffor}         % Foreach loop to build chapters
\usepackage[normalem]{ulem} % Underline for final exam and strikethrough in Omake III Alladin/Jasmine
\usepackage[bookmarks=true,unicode=true,pdfborder={0 0 0},
	pdftitle={Harry Potter and the \hpBookTitle},
	pdfauthor={LessWrong}, breaklinks={true},
	pdfkeywords={Harry Potter, rationality, \hpBookTitle},pdfencoding=auto
...
\newcommand*{\headline}[1]{\begin{center}\textsc{#1}\end{center}} % Newspaper headlin Used for start of book 5
\newcommand*{\inlineheadline}[1]{\textsc{#1}} % Newspaper headlines inline with text.

\newcommand{\chalkboard}[1]{ % chalkboard font, used in Chapter 15 for \"Transfiguration is Not Permanant"`
    {
        \vspace{0.25cm}
        \begin{center}
        \Large
